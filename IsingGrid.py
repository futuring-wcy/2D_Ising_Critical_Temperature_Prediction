...
class Grid(object):
    ...
    def __init__(self, size, Jfactor,flag):
        self.size = size
    ...
    def clusterFlip(self, temperature):
        ...
        while len(stack) > 0.5:
            ...
            if(self.flag):
                [uprightx, uprighty] = self.right(upx, upy)
                if self.canvas[uprightx, uprighty] * sign > 0.5 and \
                        lable[uprightx, uprighty] and np.random.rand() < P_add:
                    stack.append([uprightx, uprighty])
                    lable[uprightx, uprighty] = 0

                [downleftx, downlefty] = self.left(downx, downy)
                if self.canvas[downleftx, downlefty] * sign > 0.5 and \
                        lable[downleftx, downlefty] and np.random.rand() < P_add:
                    stack.append([downleftx, downlefty])
                    lable[downleftx, downlefty] = 0
